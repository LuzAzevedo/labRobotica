\begin{frame}
	\frametitle{Distribuições do ROS 2}
        
	\newcommand{\cor}{\cellcolor{red!50}}
	\only<1>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			\cor 2017 & 2018 & 2018 & 2019 & 2019 & 2020 & 2021 & 2022 & 2023 & 2024  \\
			\hline
			\cor Dec  & Jul  & Dec  & May  & Nov  & Jun  & May  & May  & May  & May   \\
			\cor 08   & 02   & 14   & 31   & 22   & 05   & 23   & 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<2>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 & \cor 2018 & 2018 & 2019 & 2019 & 2020 & 2021 & 2022 & 2023 & 2024 \\
			\hline
			Dec  & \cor Jul  & Dec  & May  & Nov  & Jun  & May  & May  & May  & May   \\
			08   & \cor 02   & 14   & 31   & 22   & 05   & 23   & 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<3>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 & 2018 & \cor 2018 & 2019 & 2019 & 2020 & 2021 & 2022 & 2023 & 2024 \\
			\hline
			Dec  & Jul  & \cor Dec  & May  & Nov  & Jun  & May  & May  & May  & May   \\
			08   & 02   & \cor 14   & 31   & 22   & 05   & 23   & 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<4>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 & 2018 & 2018 & \cor 2019 & 2019 & 2020 & 2021 & 2022 & 2023 & 2024 \\
			\hline
			Dec  & Jul  & Dec  & \cor May  & Nov  & Jun  & May  & May  & May  & May   \\
			08   & 02   & 14   & \cor 31   & 22   & 05   & 23   & 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<5>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 & 2018 & 2018 & 2019 & \cor 2019 & 2020 & 2021 & 2022 & 2023 & 2024 \\
			\hline
			Dec  & Jul  & Dec  & May  & \cor Nov  & Jun  & May  & May  & May  & May   \\
			08   & 02   & 14   & 31   & \cor 22   & 05   & 23   & 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<6>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 & 2018 & 2018 & 2019 & 2019 & \cor 2020 & 2021 & 2022 & 2023 & 2024 \\
			\hline
			Dec  & Jul  & Dec  & May  & Nov  & \cor Jun  & May  & May  & May  & May   \\
			08   & 02   & 14   & 31   & 22   & \cor 05   & 23   & 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<7>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 &  2018 & 2018 & 2019 & 2019 & 2020 & \cor 2021 & 2022 & 2023 & 2024 \\
			\hline
			Dec  & Jul  & Dec  & May  & Nov  & Jun  & \cor May  & May  & May  & May   \\
			08   & 02   & 14   & 31   & 22   & 05   & \cor 23   & 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<8>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 &  2018 & 2018 & 2019 & 2019 & 2020 & 2021 & \cor 2022 & 2023 & 2024 \\
			\hline
			Dec  & Jul  & Dec  & May  & Nov  & Jun  & May  & \cor May  & May  & May   \\
			08   & 02   & 14   & 31   & 22   & 05   & 23   & \cor 23   & 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<9>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 &  2018 & 2018 & 2019 & 2019 & 2020 & 2021 & 2022 & \cor 2023 & 2024 \\
			\hline
			Dec  & Jul  & Dec  & May  & Nov  & Jun  & May  & May  & \cor May  & May   \\
			08   & 02   & 14   & 31   & 22   & 05   & 23   & 23   & \cor 23   & 23     \\
			\hline
		\end{tabular}
	\end{table}
	}
	\only<10>
	{
	\begin{table}
		\tiny
		\begin{tabular}{| c |  c | c | c | c | c | c | c | c | c | }
			\hline
			2017 & 2018 & 2018 & 2019 & 2019 & 2020 & 2021 & 2022 & 2023 & \cor  2024 \\
			\hline
			Dec  & Jul  & Dec  & May  & Nov  & Jun  & May  & May  & May  & \cor May   \\
			08   & 02   & 14   & 31   & 22   & 05   & 23   & 23   & 23   & \cor 23     \\
			\hline
		\end{tabular}
	\end{table}
	}

	\only<1>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Ardent-Apalone.html}{\color{red} ROS \color{blue}A\color{red}rdent Apalone}} \\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Ardent Apalone.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						December, 2018 \\
						\hline
					\end{tabular}
				\end{table}	

			\end{column}		
		\end{columns}
	}
	\only<2>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Bouncy-Bolson.html}{\color{red} ROS \color{blue}B\color{red}ouncy Bolson}}\\~\\
		\begin{columns}
			\centering
			\begin{column}{0.3\textwidth}
				\includegraphics[width=3cm,height=3cm]{img02/Bouncy Bolson.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						July, 2019 \\
						\hline
					\end{tabular}
				\end{table}	

			\end{column}		
		\end{columns}
	}
	\only<3>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Crystal-Clemmys.html}{\color{red} ROS \color{blue}C\color{red}rystal Clemmys}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Crystal Clemmys.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						December, 2019 \\
						\hline
					\end{tabular}
				\end{table}	

			\end{column}		
		\end{columns}
	}
	\only<4>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Dashing-Diademata.html}{\color{red} ROS \color{blue}D\color{red}ashing Diademata}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Dashing Diademata.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						May, 2021 \\
						\hline
					\end{tabular}
				\end{table}	
			\end{column}		
		\end{columns}
	}
	\only<5>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Eloquent-Elusor.html}{\color{red} ROS \color{blue}E\color{red}loquent Elusor}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Eloquent Elusor.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						November, 2020 \\
						\hline
					\end{tabular}
				\end{table}	
			\end{column}		
		\end{columns}
	}
	\only<6>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Foxy-Fitzroy.html}{\color{red} ROS \color{blue}F\color{red}oxy Fitzroy}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Foxy Fitzroy.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						June, 2023 \\
						\hline
					\end{tabular}
				\end{table}	
			\end{column}		
		\end{columns}
	}
	\only<7>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Galactic-Geochelone.html}{\color{red} ROS \color{blue}G\color{red}alactic Geochelone}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Galactic Geochelone.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						December, 2022 \\
						\hline
					\end{tabular}
				\end{table}		
			\end{column}		
		\end{columns}
	}
	\only<8>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Humble-Hawksbill.html}{\color{red} ROS \color{blue}H\color{red}umble Hawksbill}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Humble Hawksbill.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						May, 2027 \\
						\hline
					\end{tabular}
				\end{table}		
			\end{column}		
		\end{columns}
	}
	\only<9>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Iron-Irwini.html}{\color{red} ROS \color{blue}I\color{red}ron Irwini}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Iron Irwini.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						December, 2024 \\
						\hline
					\end{tabular}
				\end{table}		
			\end{column}		
		\end{columns}
	}
	\only<10>
	{
		\textbf{\href{https://docs.ros.org/en/jazzy/Releases/Release-Jazzy-Jalisco.html}{\color{red} ROS \color{blue}J\color{red}azzy Jalisco}}\\~\\
		\begin{columns}
			\begin{column}{0.3\textwidth}
				\centering
				\includegraphics[width=3cm,height=3cm]{img02/Jazzy Jalisco.png}
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
			\end{column}
			\begin{column}{0.3\textwidth}
				\centering
				\begin{table}
					\tiny
					\begin{tabular}{|c|}
						\hline
						End-of-Life (EOL) \\
						\hline
						May, 2029 \\
						\hline
					\end{tabular}
				\end{table}		
			\end{column}		
		\end{columns}
	}


\end{frame}
